                             ...
                             GTask        *task)
{
    ...
    /* If context is already active, deactivate it before attempting to configure and connect */
...
build_auth_string (GTask *task)
{
    ...
    ctx = (Dial3gppContext *) g_task_get_task_data (task);
    self = (MMBroadbandBearerInfineon *) g_task_get_source_object(task);
...
dial_3gpp_context_step (GTask *task)
{
    ...
    case DIAL_3GPP_CONTEXT_STEP_DEACT: {
        /* Disconnect so that we can reconfigure a context that is already activated. */
        gchar *command;
        ...
        if (ctx->cid != 1)
        {
            ...
            /* Skip this step if we are connecting via the default bearer with cid == 1 */
            ctx->step++;
        }
    }
           ...
           gpointer user_data)
{
    ...
    ctx->usb_interface_config_index = mm_kernel_device_get_property_as_int(mm_port_peek_kernel_device(ctx->data), "ID_MM_INFINEON_NCM_ID");
    ctx->usb_acm_config_index = mm_kernel_device_get_property_as_int(mm_port_peek_kernel_device(ctx->data), "ID_MM_INFINEON_ACM_ID");
               ...
               GTask *task)
{
    ...
    /* Parse response */
    if (!mm_infineon_parse_cgpaddr_response (response, &cid, &ipv4addr, &ipv6addr, &error)) {
        g_task_return_error (task, error);
        g_object_unref (task);
        return;
    }
                ...
                GTask *task)
{
    ...
    if (!mm_infineon_parse_xdns_query_response (response, ctx->cid, &dns_v4, &dns_v6, &error)) {
...
get_ip_config_3gpp_context_step (GTask *task)
{
    ...
    mm_dbg ("running infineon get ip config step %u/%u...", ctx->step, GET_IP_CONFIG_3GPP_CONTEXT_STEP_LAST);
...
disconnect_3gpp_context_step (GTask *task)
{
    ...
    mm_dbg ("running infineon disconnect step %u/%u...", ctx->step, DISCONNECT_3GPP_CONTEXT_STEP_LAST);
