...
static void
test_mm_xmm_parse_cgpaddr_response (void)
{
    guint i;
    ...
    for (i = 0; i < G_N_ELEMENTS (cgpaddr_response_tests); i++) {
        ...
        success = mm_xmm_parse_cgpaddr_response    (cgpaddr_response_tests[i].str,
...
static void
test_mm_xmm_parse_xdns_query_response (void)
{
    guint i, j;
    ...
    for (i = 0; i < G_N_ELEMENTS (xdns_response_tests); i++) {
        ...
        success = mm_xmm_parse_xdns_query_response (xdns_response_tests[i].str,
...
int main (int argc, char **argv)
{
    ...
    g_test_add_func ("/MM/xmm/xdns/query_response", test_mm_xmm_parse_xdns_query_response);
    g_test_add_func ("/MM/xmm/cgpaddr/query_response", test_mm_xmm_parse_cgpaddr_response);
